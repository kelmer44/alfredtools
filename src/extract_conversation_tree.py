...
def format_tree_output(tree: ConversationTree) -> List[str]:
    ...
    for i, root in enumerate(tree.roots):
        format_node(root, lines, prefix="", is_last=True, is_root=True, root_display_index=i)
        lines.append("")
...
def format_node(node: ConversationNode, lines: List[str], prefix: str, is_last: bool, is_root: bool = False, root_display_index: Optional[int] = None) -> None:
    """Recursively format a node with tree characters"""

    # Determine tree characters
    if is_root:
        root_info = f" #{root_display_index}" if root_display_index is not None else ""
        connector = f"└─ ROOT{root_info}"
        child_prefix = "   "
    else:
    ...
    if node.node_type == 'root':
        ...
        lines.append(f"{child_prefix}NPC: {node.text}")
        ...
        # NPC response
        action_str = f" → ACTION {node.action_code:04X} ({node.action_code})" if node.action_code else ""
