...
def main():
    ...
    for start_buda in range(len(budas) - 1):
        ...
        isPalette = False
        if start_buda in palettes:
            print(f"Buda {start_buda} isPalette!")
            isPalette = True
        for i in range(start_buda, min(start_buda + 20, len(budas) - 1)):
            # if i in palettes and i != start_buda:
            ...
            if len(combined) >= TARGET_SIZE * 1.2:  # Close enough
                break

        # Check if we got close to the target size
        if TARGET_SIZE * 0.85 <= len(combined) <= TARGET_SIZE * 2:
            ...
            if pal_buda:
                ...
                if isPalette:
                    startOffset = 13252
                # Create image
                img_data = bytes(combined[startOffset:(startOffset + TARGET_SIZE)])
